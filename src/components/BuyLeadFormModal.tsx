...
export default function BuyLeadFormModal({ isOpen, onClose, onSubmit, initialData }: BuyLeadFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-end justify-center z-50">
      <div className="bg-white dark:bg-slate-900 rounded-t-3xl w-full max-h-[88%] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
        {/* Header bar */}
        <div className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800 px-5 py-4 flex items-center justify-between">
          <div>
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-50 tracking-tight flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Get instant quotes from multiple verified suppliers</span>
          </div>
          <button onClick={onClose} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 rounded-full bg-slate-100 dark:bg-slate-800">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Product Name *</label>
            <input
              type="text"
              value={productName}
              onChange={(e) => setProductName(e.target.value)}
              placeholder="E.g., Centrifugal Pump"
              className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 focus:border-accent-blue focus:bg-white dark:focus:bg-slate-900 rounded-xl px-3 py-2 text-xs outline-none transition"
            />
          </div>

          {/* Brand Name (Optional) */}
          <div className="space-y-1">
            <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Brand Specific (Optional)</label>
            <input
              type="text"
              value={brandName}
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Quantity */}
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Estimated Quantity *</label>
              <input
                type="text"
                value={quantity}
            ...
            {/* Location */}
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Delivery Location *</label>
              <input
                type="text"
                value={location}
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Detailed Specifications *</label>
            <textarea
              rows={4}
              value={requirement}
              ...
              className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 focus:border-accent-blue focus:bg-white dark:focus:bg-slate-900 rounded-xl px-3 py-2 text-xs outline-none transition resize-none leading-relaxed"
            />
          </div>
