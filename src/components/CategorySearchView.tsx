    ...
    <div className="flex-1 bg-canvas flex flex-col overflow-hidden select-none font-sans text-slate-800 dark:text-slate-200">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 p-4 space-y-3 shrink-0">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-300 focus:border-accent-blue focus:bg-white dark:focus:bg-slate-900 rounded-xl pl-9 pr-4 py-2.5 text-xs outline-none transition font-semibold"
          />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {filteredCategories.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center text-slate-400 dark:text-slate-500 text-xs">
          ...
          filteredCategories.map((cat) => {
            ...
            return (
              <Link
                ...
                className="block bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl overflow-hidden shadow-xs hover:border-accent-blue/40 transition duration-200"
              >
                ...
                <div className="p-3.5 bg-slate-50/50 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-extrabold text-[12px] text-slate-900 dark:text-slate-50 leading-tight truncate">
                        {cat.name}
                      </h3>
                      <span className="text-[8.5px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mt-0.5">
                        {brandCount > 0
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className={`p-2 rounded-lg border transition ${
                        isShortlisted
                          ...
                          : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-rose-500 hover:bg-slate-50 dark:hover:bg-slate-800/60'
                      }`}
                    ...
                    <div className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 rounded-lg">
                ...
                {topBrands.length > 0 && (
                  <div className="px-3.5 py-2.5 bg-white dark:bg-slate-900 flex items-center gap-2">
                    <div className="flex -space-x-2 shrink-0">
                      {topBrands.map((b) => (
                        <div key={b.id} className="w-6 h-6 rounded-full bg-white dark:bg-slate-900 ring-2 ring-white border border-slate-200 dark:border-slate-700 overflow-hidden flex items-center justify-center shrink-0">
                          <BrandLogo logo={b.logo} name={b.name} className="w-full h-full object-contain p-0.5" />
                        </div>
                      ))}
                    </div>
                    <span className="text-[9.5px] font-semibold text-slate-500 dark:text-slate-400 truncate">
