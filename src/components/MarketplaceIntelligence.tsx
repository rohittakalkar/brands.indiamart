...
export default function MarketplaceIntelligence() {
  return (
    <div className="flex-1 bg-[#f4f6f8] overflow-y-auto pb-12 text-slate-800 font-sans select-none">
      {/* Header Banner */}
      <div className="relative bg-gradient-to-r from-[#028384] to-[#005e60] px-4 py-6 text-center overflow-hidden text-white">
        <div className="absolute top-0 right-0 w-48 h-48 bg-white/5 rounded-full blur-3xl"></div>
        
        <div className="relative flex flex-col items-center">
          <span className="px-2.5 py-0.5 bg-white/10 border border-white/20 text-teal-200 rounded-full text-[9px] font-bold tracking-widest uppercase mb-1 flex items-center gap-1">
          ...
          <p className="text-[10px] text-teal-50 mt-1 max-w-xs leading-normal">
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 flex flex-col justify-between shadow-xs">
            <div className="flex items-center justify-between text-slate-400">
              <Users className="w-4 h-4 text-[#028384]" />
              <span className="text-[8.5px] font-extrabold bg-teal-50 text-[#028384] px-1.5 py-0.5 rounded border border-teal-100">Monthly</span>
            </div>
            <div className="mt-2.5">
              <div className="text-base font-extrabold text-slate-900 tracking-tight">{MARKETPLACE_METRICS.activeBuyers}</div>
              <div className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-wider">Active B2B Buyers</div>
            </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 flex flex-col justify-between shadow-xs">
            ...
            <div className="mt-2.5">
              <div className="text-base font-extrabold text-slate-900 tracking-tight">{MARKETPLACE_METRICS.buyLeadsThisMonth}</div>
              <div className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-wider">RFQs & BuyLeads</div>
            </div>
            <div className="text-[8px] text-emerald-600 font-bold flex items-center gap-0.5 mt-1">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 flex flex-col justify-between shadow-xs">
            <div className="flex items-center justify-between text-slate-400">
              ...
              <span className="text-[8.5px] font-extrabold bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded border border-blue-100">Traffic</span>
            </div>
            <div className="mt-2.5">
              <div className="text-base font-extrabold text-slate-900 tracking-tight">{MARKETPLACE_METRICS.productViews}</div>
              <div className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-wider">Product Views</div>
            </div>
            <div className="text-[8px] text-emerald-600 font-bold flex items-center gap-0.5 mt-1">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 flex flex-col justify-between shadow-xs">
            ...
            <div className="mt-2.5">
              <div className="text-base font-extrabold text-slate-900 tracking-tight">{MARKETPLACE_METRICS.verifiedSuppliers}</div>
              <div className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-wider">Verified OEMs</div>
            </div>
            <div className="text-[8px] text-emerald-600 font-bold flex items-center gap-0.5 mt-1">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-4 space-y-3 shadow-xs">
          <div className="flex items-center gap-1.5 text-slate-800">
            <BarChart3 className="w-4 h-4 text-[#028384]" />
            <h2 className="text-xs font-extrabold uppercase tracking-wider">Top Trending Categories (By BuyLeads)</h2>
          </div>
            ...
            {TRENDING_CATEGORIES.map((cat, idx) => (
              <div key={idx} className="space-y-1">
                ...
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200/20">
                  <div 
                    className="h-full bg-[#028384] rounded-full" 
        ...
        <div className="space-y-4">
              ...
              {MOST_INQUIRED_PRODUCTS.map((prod, idx) => (
                <div key={idx} className="py-2.5 flex items-center justify-between text-[11px] first:pt-0 last:pb-0">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-400 text-[10px]">{idx + 1}</span>
                    <span className="font-bold text-slate-700">{prod.name}</span>
                  </div>
                  <div className="text-right">
                    <div className="font-extrabold text-slate-900">{prod.count}</div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-4 space-y-3 shadow-xs">
            <div className="flex items-center gap-1.5 text-slate-800">
              <MapPin className="w-4 h-4 text-[#028384]" />
              <h2 className="text-xs font-extrabold uppercase tracking-wider">Top Regions (BuyLeads Share)</h2>
            </div>
            <div className="space-y-3.5">
              {REGIONAL_DEMAND.map((reg, idx) => (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between text-[11px] font-semibold text-slate-700">
                    ...
                    <span className="font-extrabold text-[#028384]">{reg.share}</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/10">
                    <div 
                      className="h-full bg-gradient-to-r from-[#028384] to-[#005e60] rounded-full" 
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 border border-slate-900 rounded-2xl p-4 space-y-3 text-white shadow-sm">
          <div className="text-xs font-extrabold uppercase tracking-wider text-teal-200">Buyer Intent Signals (Monthly)</div>
          <div className="grid grid-cols-2 gap-3">
            {BUYER_INTENT_SIGNALS.map((sig, idx) => (
              <div key={idx} className="bg-white/5 border border-white/10 rounded-xl p-3 text-center">
                <div className="text-base font-extrabold text-teal-300">{sig.volume}</div>
                <div className="text-[9px] text-slate-300 font-medium mt-0.5">{sig.signal}</div>
